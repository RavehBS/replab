...
class WidowX:
    ...
    def move_to_joint_values(self, joint):
        for i in range(5):
            if joint[i] >= JOINT_MAX[i]:
                joint[i] = JOINT_MAX[i]
        if joint[i] <= JOINT_MIN[i]:
            joint[i] = JOINT_MIN[i]
        plan = self.commander.plan(joint)
        return self.commander.execute(plan, wait=True)
        
    def m(self, joint): #remove debug
        for i in range(5):
            if joint[i] >= JOINT_MAX[i]:
                joint[i] = JOINT_MAX[i]
        if joint[i] <= JOINT_MIN[i]:
    ...
    def execute_grasp(self, grasp):
        try:
            ...
            assert self.orient_to_pregrasp(
                x, y), 'Failed to orient to target'

            assert self.move_to_grasp(x, y, PRELIFT_HEIGHT, theta), \
                'Failed to reach pre-lift pose'

            assert self.move_to_grasp(
                x, y, z, theta), 'Failed to execute grasp'

            #assert self.move_to_drop(), 'Failed to move to drop'
    ...
    def calibration_positions(self):
        for j in range(5):
            for i in range(5):
                ...
                key = ''
                while key != 't':
                    key = raw_input('press t to continue, x to quit.\n') #wait
                    if key == 'x':
                        reached = self.move_to_vertical(PRELIFT_HEIGHT)
                        return
...
def main():
    ...
    widowx = WidowX()
    #widowx.move_to_joint_values([0, 0, 0, 0, 0])
...
"""
    def p(self):
        self.move_to_reset()
        self.move_to_joint_values(OCI1) #crane, intermidiate.
        self.move_to_joint_values(OCI2) #start of outer circle front
        target = OCI2
        for i in range(10):
            target[0] = -3 + i* (0.0-(-3.0))/10  #10 steps
            self.move_to_joint_values(target)
            key = ''
            while key != 't':
                key = raw_input('press t\n') #wait
                if key == 'x':
                    return
        self.move_to_joint_values(OCI3) #prepare to go to move_to_reset
        self.move_to_reset()
        self.move_to_joint_values(OCI1) #crane, intermidate
        self.move_to_joint_values(OCI4) #start of outer circle, back.
        target = OCI4
        for i in range(10):
            target[0] = -3 + i* (-0.5-(-3.0))/10 #10 steps
            self.move_to_joint_values(target)
            key = ''
            while key != 't':
                key = raw_input('press t\n') #wait
                if key == 'x':
                    return
        self.move_to_joint_values(OCI4) #start of outer circle, back.
        self.move_to_joint_values(OCI1) #start of outer circle, back.
        self.move_to_reset()
        return
 """
